...
export async function actorFromTelegram(telegramUserId:number){const rows=await querySystem<Actor&{displayName:string;chatId:number}>(`select p.id,p.status::text,p.role::text,p.display_name as "displayName",b.telegram_chat_id as "chatId" from public.bot_connections b join public.profiles p on p.id=b.user_id where b.telegram_user_id=$1`,[telegramUserId]);return rows[0]??null;}
