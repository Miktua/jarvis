...
import { UserRulesPanel } from "@/components/user-rules/user-rules-panel";
...
export default async function WorkspacePage({searchParams}:{searchParams:Promise<{table?:string}>}){
  ...
  const tables=await listTables(actor) as WorkspaceTable[]; const requested=(await searchParams).table; const userRules=Array.isArray(actor.aiRules)?actor.aiRules as Array<{id:string;instruction:string;enabled:boolean}>:[];
