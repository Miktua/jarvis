...
type WorkspaceTable={id:string;display_name:string;description:string;access:"owner"|"read"|"write";schema_definition:Array<{id:string;label:string}>};
...
export default async function WorkspacePage({searchParams}:{searchParams:Promise<{table?:string}>}){
  ...
  return <main className="workspace">
    <aside><div className="workspace-brand"><span>J</span><strong>Jarvis</strong></div>
    ...
    <section className="content"><header className="content-header"><div><p className="eyebrow">{selected?.access?.toUpperCase()??"WORKSPACE"}</p><h1>{selected?.display_name??"Your workspace"}</h1><p className="muted">{selected?.description??"No tables are available."}</p>{selected?.access==="owner"&&<PermissionsPanel tableId={selected.id}/>}</div><div className="profile-chip"><span>{actor.displayName.slice(0,1).toUpperCase()}</span><div><strong>{actor.displayName||"User"}</strong><small>{actor.role}</small></div></div></header>{selected&&<DataGrid columns={selected.schema_definition} rows={rows as Record<string,unknown>[]}/>}</section>
  </main>;
}
